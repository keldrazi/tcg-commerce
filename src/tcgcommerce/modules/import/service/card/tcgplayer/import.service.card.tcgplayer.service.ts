...
import { IMPORT_JOB_CARD_STATUS, IMPORT_SORT_CARD_CONDITION, IMPORT_SORT_CARD_PRINTING, IMPORT_SORT_TCGPLAYER_CARD_DATA_KEYS } from 'src/system/constants/tcgcommerce/import/job/card/tcgcommerce.import.job.card.constants';
...
@Injectable()
export class ImportServiceCardTCGPlayerService {
    ...
    async processImportCards(importJobCardFile: Express.Multer.File) {
        ...
        for(let i = 0; i < importSortCardData.importCardData.length; i++) {
            ...
            if(cardData[IMPORT_SORT_TCGPLAYER_CARD_DATA_KEYS.CARD_TCG_PLAYER_ID] != '') {

                let importSortCardDTO = new ImportSortCardDTO();
                let importCardTCGPlayerId = cardData[IMPORT_SORT_TCGPLAYER_CARD_DATA_KEYS.CARD_TCG_PLAYER_ID];
                importSortCardDTO.importSortCardSetName = cardData[IMPORT_SORT_TCGPLAYER_CARD_DATA_KEYS.CARD_SET_NAME];
                importSortCardDTO.importSortCardName = cardData[IMPORT_SORT_TCGPLAYER_CARD_DATA_KEYS.CARD_NAME];
                importSortCardDTO.importSortCardNumber = cardData[IMPORT_SORT_TCGPLAYER_CARD_DATA_KEYS.CARD_NUMBER];
                importSortCardDTO.importSortCardCondition = await this.importServiceCardUtilService.getImportSortCardCondition(cardData[IMPORT_SORT_TCGPLAYER_CARD_DATA_KEYS.CARD_CONDITION]);
                //NEED TO CHECK THIS;
                importSortCardDTO.importSortCardPrinting = IMPORT_SORT_CARD_PRINTING.NORMAL;
                importSortCardDTO.importSortCardTCGPlayerMarketPrice = parseFloat(cardData[IMPORT_SORT_TCGPLAYER_CARD_DATA_KEYS.CARD_TCG_PLAYER_MARKET_PRICE]);
                importSortCardDTO.importSortCardTCGPlayerLowPrice = parseFloat(cardData[IMPORT_SORT_TCGPLAYER_CARD_DATA_KEYS.CARD_TCG_PLAYER_LOW_PRICE]);
                importSortCardDTO.importSortCardQty = parseInt(cardData[IMPORT_SORT_TCGPLAYER_CARD_DATA_KEYS.CARD_QTY]);
    ...
    async processCSVCardData(importCSVCardData: any) {
        ...
        for(let i = 0; i < importCSVCardData.length; i++) {
            ...
            let importCardQty = parseInt(importCard[IMPORT_SORT_TCGPLAYER_CARD_DATA_KEYS.CARD_QTY]);
            let importCardTCGPlayerMarketPrice = parseFloat(importCard[IMPORT_SORT_TCGPLAYER_CARD_DATA_KEYS.CARD_TCG_PLAYER_MARKET_PRICE]) * importCardQty;
            let importCardTCGPlayerLowPrice = parseFloat(importCard[IMPORT_SORT_TCGPLAYER_CARD_DATA_KEYS.CARD_TCG_PLAYER_LOW_PRICE]) * importCardQty;
