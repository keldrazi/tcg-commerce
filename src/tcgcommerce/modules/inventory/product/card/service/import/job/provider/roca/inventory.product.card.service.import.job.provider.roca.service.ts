...
import { InventoryProductCardServiceImportJobTypeUtilService } from 'src/tcgcommerce/modules/inventory/product/card/service/import/job/provider/util/inventory.product.card.service.import.job.provider.util.service';
...
export class InventoryProductCardServiceImportJobTypeRocaService {
    ...
    async InventoryProductCardServiceImportJobCards(inventoryProductCardServiceImportJobFile: Express.Multer.File) {
        ...
        for(let i = 0; i < inventoryProductCardServiceImportJobData.inventoryProductCardServiceImportJobCardData.length; i++) {
            ...
            if(inventoryProductCardServiceImportJobCardData[INVENTORY_PRODUCT_CARD_SERVICE_IMPORT_JOB_TYPE_ROCA_DATA_KEYS.CARD_TCG_PLAYER_ID] != '') {

                let inventoryProductCardServiceImportJobTypeRocaDTO = new InventoryProductCardServiceImportJobTypeRocaDTO();
                inventoryProductCardServiceImportJobTypeRocaDTO.inventoryProductCardServiceImportJobTypeRocaTCGPlayerId = inventoryProductCardServiceImportJobCardData[INVENTORY_PRODUCT_CARD_SERVICE_IMPORT_JOB_TYPE_ROCA_DATA_KEYS.CARD_TCG_PLAYER_ID];
                inventoryProductCardServiceImportJobTypeRocaDTO.inventoryProductCardServiceImportJobTypeRocaCondition = await this.inventoryProductCardServiceImportJobTypeUtilService.getInventoryProductCardServiceImportJobTypeCardCondition(inventoryProductCardServiceImportJobCardData[INVENTORY_PRODUCT_CARD_SERVICE_IMPORT_JOB_TYPE_ROCA_DATA_KEYS.CARD_CONDITION]);
                inventoryProductCardServiceImportJobTypeRocaDTO.inventoryProductCardServiceImportJobTypeRocaPrinting = await this.inventoryProductCardServiceImportJobTypeUtilService.getInventoryProductCardServiceImportJobTypeCardPrinting(inventoryProductCardServiceImportJobCardData[INVENTORY_PRODUCT_CARD_SERVICE_IMPORT_JOB_TYPE_ROCA_DATA_KEYS.CARD_CONDITION]);
                inventoryProductCardServiceImportJobTypeRocaDTO.inventoryProductCardServiceImportJobTypeRocaQty = parseInt(inventoryProductCardServiceImportJobCardData[INVENTORY_PRODUCT_CARD_SERVICE_IMPORT_JOB_TYPE_ROCA_DATA_KEYS.CARD_QTY]);
    ...
    async processInventoryProductCardServiceImportJobCSVData(inventoryProductCardServiceImportJobCSVData: any) {
        ...
        for(let i = 0; i < inventoryProductCardServiceImportJobCSVData.length; i++) {
            ...
            let inventoryProductCardServiceImportJobQty = parseInt(inventoryProductCardServiceImportJobCard[INVENTORY_PRODUCT_CARD_SERVICE_IMPORT_JOB_TYPE_ROCA_DATA_KEYS.CARD_QTY]);
